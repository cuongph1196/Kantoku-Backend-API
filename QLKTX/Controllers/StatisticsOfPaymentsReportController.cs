...
{
    [Route("StatisticsOfPaymentsReport")]
    [ApiController]
    [Authorize]
    public class StatisticsOfPaymentsReportController : BaseController
    {
        public static IWebHostEnvironment _environment;
        private readonly ILogger<StatisticsOfPaymentsReportController> _logger;
        private readonly IStatisticsOfPaymentsRepository _vdrRepository;
        private IMemoryCache _cache;
        public StatisticsOfPaymentsReportController(ILogger<StatisticsOfPaymentsReportController> logger,
            IMemoryCache cache,
            IStatisticsOfPaymentsRepository vdrRepository,
        ...
        public dynamic SearchReport([FromQuery] string FromDate, string ToDate, string RenderType) //[FromQuery] SummaryVoucherRSearchVm viewModel
        {
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                string mineType = "";
                int extension = 1;
                var path = $"{_environment.WebRootPath}\\Reports\\rpt_StatisticsOfPaymentsReport.rdlc";

                Dictionary<string, string> parameters = new Dictionary<string, string>();
                LocalReport localR = new LocalReport(path);
                localR.AddDataSource("ds_StatisticsOfPayments", dt);
