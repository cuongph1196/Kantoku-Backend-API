			...
			$.map(rows, function (row, index) {
				...
				for (var i = 0; i < fields.length; i++){
					if (index == 0) {
						...
						var bindex = index - 1;
						var brow = rows[bindex];
						debugger
						if (i == 0) {
							value = (parseFloat(brow[opts.pivot.aggregate.column.field]) || 0) + (parseFloat(row._rows[0].RecordID == 'PTHU' ? row[fields[i]] : -Math.abs(row[fields[i]])) || 0);
						} else {
							value += (parseFloat(row._rows[0].RecordID == 'PTHU' ? row[fields[i]] : -Math.abs(row[fields[i]])) || 0);
                        }
                    }
				}
