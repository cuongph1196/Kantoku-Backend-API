...
{
    public interface IContractStatisticsReasonReportRepository
    {
        List<IDictionary<string, object>> SearchReport(ReportSearchVm vm);
        dynamic SearchReportv2(ReportSearchVm vm);
    }
    public class ContractStatisticsReasonReportRepository : IContractStatisticsReasonReportRepository
    {
        private readonly string _connectionString;
        private readonly ConnectionStrings _connectionStrings;

        public ContractStatisticsReasonReportRepository(IConfiguration configuration, IOptions<ConnectionStrings> connectionStrings)
        {
        ...
        public List<IDictionary<string, object>> SearchReport(ReportSearchVm vm)
        {
            ...
            var result = new StoredProcedureFactory<dynamic>(_connectionString).FindAllBy(masterParams,
                "sprpt_ContractStatisticsReasonReport", "SearchDetail");
